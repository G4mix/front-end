...
export class PostQueryManager extends APIManager {
  public static async findPostById(
    ...
    const query: GenericQueryRequest<"findPostById"> = {
      query: "query findPostById($id: Int!) { findPostById(id: $id) { id author { id displayName user { id, username, email, icon } } title content createdAt updatedAt likesCount commentsCount viewsCount tags { id name } links { id link } images { id name src width height } }}",
      variables: {
        id: id
      }
    };
  ...
  public static async findAllPosts(
    ...
    const query: GenericQueryRequest<"findAllPosts"> = {
      query: "query findAllPosts($skip: Int, $limit: Int) { findAllPosts(skip: $skip, limit: $limit) { id author { id displayName user { id, username, email, icon } } title content createdAt updatedAt likesCount commentsCount viewsCount tags { id name } links { id link } images { id name src width height } }}",
      variables: {
        skip: skip,
        limit: 10
      }
